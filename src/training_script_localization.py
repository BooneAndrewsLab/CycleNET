...
parser.add_argument("-n", "--use_non_cropped", action="store_true", help="Use non cropped cells")
...
def inference_oren(input_images,is_training):
    ...
    fc_1 = nn_layers.nn_layer(pool3_flat, 8 * 8 * 64, 1024, 'fc_1', act=tf.nn.relu, is_training=is_training)
    logit = nn_layers.nn_layer(fc_1, 1024, NUM_CLASSES, 'final_layer', act=None, is_training=is_training)
